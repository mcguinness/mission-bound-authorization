...
Implementation: 6 conformance rows in conformance-manifest.json (3 tested, 2 partial, 1 todo).
...
This document remains complete on its own: an issuer MAY
...
- MUST carry the `mission` claim
  ...
  Claim") with `id` and `issuer` unchanged from the Mission, and the
  audience-scoped `authorization_details` ({{audience-scope}}). This
  profile REQUIRES `authority_hash` on that claim too, reintroducing
  it beyond the issuance profile's baseline as this profile's own
  lineage and audit anchor for the party receiving the grant across
  the trust-domain hop; and
...
: REQUIRED on a credential, delegation chain, or projected local
  ...
  a serialization is not the test.

  It is identity provenance and a
  ...
  No display name, email, organization, role, group,
...
the invariant forbids per-destination substitution afterward.
...
decision, never a bearer bypass.
...
This is the profile's conformance
...
Cross-domain revocation latency is therefore the
...
- issuer trust is established by local policy or
  metadata, never inferred from a signed assertion's own
  `mission.issuer`;
- the grant is sender-constrained and one-time-use,
  because the underlying grant profile provides no replay backstop of
  its own; and
- everything the Resource AS honors is bounded by the
...
({{validation-at-resource-as}}). This local depth is distinct
...
projected token cannot re-enter a chain. An entry's `max_depth` ceiling is
