...
Authorization Server MUST NOT thereafter derive a token carrying that
entry, exactly as it refuses derivation for a non-`active` Mission.
...
Mission completes one entry at a time while the Mission remains
`active` for the rest. It also strengthens the kill switch: a task
that finishes stops issuing its own authority without waiting for a
clock or a revoke. It is the enforceable counterpart of the inert
`success_criteria`.
...
({{mission-lifecycle-endpoint}}), but this section does not require
...
machine's event sources remain authoritative ({{state-machine}}).
...
operation ({{mission-status}}) and the token introspection projection
({{introspection-projection}}) are deployed, they MUST omit a
