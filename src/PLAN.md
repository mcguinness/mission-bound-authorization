...
- **O-38. Child-delegation (parent→child sub-missions) not implemented.** The
  ...
  child grant issuance + discovery (#374). Remaining deferred: cross-issuer, PR4b
  (child redeems the assertion as itself at `/token`), and relocating child
  creation from the dev-guarded `POST /child-missions` route to `/token` via
  `registerGrantType` (PR4b and the route move both need child-actor client
  registration). Final gate 273 passed / 1 skipped.
