...
| `draft-mcguinness-mission-runtime-evidence` | `3f033485` (2026-09-08; the post-permit disposition path: five added Execution Evidence `error` values, five added pre-decision `denial_reason` values, the duplicate-identifier separation and the enumerated-value-not-diagnostic rule. The phase carriers are implemented, #252 C; the evaluation-context extension stays specified with implementation pending #773; the unavailable-target `effective_parameter_digest` encoding is open with #594 and one exit ships recordless because of it) | `services/pdp/src/{decision-evidence.ts,evaluate.ts,runtime-evidence-integrity.ts}` (Decision Evidence emission), `services/mcp-payments/src/evidence.ts` (retention, Refusal Records, Execution Evidence), `services/mcp-payments/src/{pep,server,transaction}.ts` (one post-permit suppression writer, per-attempt execution identity, call-site classification) | `runtime-evidence#decision-evidence-object`, `runtime-evidence#pre-decision-refusal`, `runtime-evidence#execution-evidence-object`, `runtime-evidence#decision-evidence-integrity` (the emitter constructs the record on its own emission path; a receiving component verifies and retains it as received) | `services/pdp/test/{decision-evidence,runtime-evidence-integrity}.test.ts`, `services/mcp-payments/test/{decision-evidence-retention,refusal-evidence,enforcement,pep-fail-closed,transaction,capability-binding,mission-receipt}.test.ts` |
