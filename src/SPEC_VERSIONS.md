...
|---|---|---|---|---|
| `draft-mcguinness-mission-control-plane` | `ae362420` (generated status only; normative text unchanged from `efb2a01b`) | `packages/store`, `packages/mission-signals`, `services/authorization-server` | `control-plane#serialization`, `control-plane#isolation`, `control-plane#fresh-observation`, `control-plane#fanout`, `control-plane#tombstones`, `control-plane#deployment-declaration`; single-process transaction, issuer-namespace and Status List republication coverage, durable lifecycle fan-out with per-subscriber acceptance and boot replay, and composed terminal tombstones with permanent identifier nonreuse; the observation/commit watermark, the Status conditional-request surface, asynchronous artifact issuance, refusal under uncertain recovery and multi-process claiming remain incomplete; deployment limits in `docs/control-plane-deployment.md` | `packages/store/test/serialization.test.ts`, `packages/mission-signals/test/durable-delivery.test.ts`, `services/authorization-server/test/{control-plane-faults,control-plane-fanout,child-delegation,expansion-endpoint,status-list}.test.ts` |
