...
|---|---|---|---|---|
| `draft-mcguinness-oauth-mission` | `aa7ad294` (2026-08-22) | `packages/mission-core` | `mission#integrity-anchors`, `mission#canonicalization`, `mission#test-vectors` | `packages/mission-core/test/anchors.test.ts` |
| `draft-mcguinness-oauth-mission` | `aa7ad294` (2026-08-22) | `services/authorization-server` (kernel + adapters) | `mission#submission-via-par`, `mission#mission-intent`, `mission#authorization-derivation`, `mission#subset`, `mission#integrity-anchors`, `mission#the-mission-claim`, `mission#lifecycle`, `mission#introspection`, `mission#as-metadata` | `services/authorization-server/test/{kernel,tracer,authorization-endpoint}.test.ts` |
| `draft-mcguinness-oauth-mission-status` | `2399f17` (2026-08-17) | `services/authorization-server` (kernel + adapters, incl. `kernel/discharge.ts`, `kernel/lifecycle-idempotency.ts`) | `status#legal-transitions`, `status#state-machine`, `status#mission-status-response`, `status#status-list`, `status#mission-status-anti-oracle`, `status#mission-status-errors`, `status#mission-status-authentication`, `status#idempotency`, `status#completion`, `status#terminal-when`, `status#subset-extension`, `status#discharge`, `status#determining`, `status#visibility`, `status#discharge-operation`, `status#discharge-authority`, `status#discharge-anti-oracle`, `status#discharge-idempotency`, `status#discharge-result`, `status#completion-security` | `services/authorization-server/test/{kernel,tracer,status-list,discharge}.test.ts` |
...
| `draft-mcguinness-mission-authzen` (context.actor) | `3730cc22` (2026-08-21) | `packages/actor-chain` | `authzen#context-actor` (root-to-leaf projection, PEP build / PDP validate, D31) | `packages/actor-chain/test/actor-chain.test.ts` |
...
| `draft-mcguinness-oauth-mission` (RS enforcement) | `aa7ad294` (2026-08-22) | `services/mcp-payments` | `mission#rs-enforcement` (token + mission claim + DPoP cnf validation, mission-scoped tools/list) | `services/mcp-payments/test/enforcement.test.ts` |
...
| `draft-mcguinness-oauth-mission-management` (partial) | `1c942878` (2026-08-21) | `services/console-bff`, `authorization-server` (allMissions) | fleet enumeration + operator lifecycle surfaces | `services/console-bff/test/console.test.ts` |
...
| `draft-mcguinness-mission-shaping` | `3730cc22` (2026-08-21) | `services/agent/src/index.ts` (shapeIntent) | untrusted intent proposal; derivation still bounds | `services/agent/test/harness.test.ts` |
...
| `draft-mcguinness-mission-orchestration` | `58c7a857` (2026-08-21) | `packages/orchestration` + `packages/mission-core` (anchors `UNWIND_PLAN_TYP`) | `orchestration#reversibility`, `orchestration#unwind-plan`, `orchestration#unwind-plan-integrity`, `orchestration#state-change-behavior`, `orchestration#compensation`, `orchestration#orchestration-evidence` | `packages/orchestration/test/orchestration.test.ts` |
...
| `draft-mcguinness-mission-substrate` | `af1782ed` (2026-08-22) | `services/authorization-server/src/kernel/{kernel,mission-id}.ts` | `mission-substrate#basic-gate`, `mission-substrate#approved-context`, `mission-substrate#actor-binding`, `mission-substrate#reference` | `services/authorization-server/test/kernel.test.ts` |
| `draft-mcguinness-mission-runtime-evidence` | `41186df2` (2026-08-21) | `services/mcp-payments/src/evidence.ts` | `runtime-evidence#decision-evidence-object`, `runtime-evidence#pre-decision-refusal` | `services/mcp-payments/test/{refusal-evidence,enforcement}.test.ts` |
| `draft-mcguinness-oauth-mission-containment` (partial) | `58c7a857` (2026-08-21) | `services/authorization-server/src/kernel/{containment,kernel,issuer-evidence}.ts` + `services/pdp/src/{evaluate,policy-view}.ts` + `packages/mission-signals` | `containment#containment-policy`, `containment#containment-plane`, `containment#propagation`, `containment#restoration`, `containment#protected-events` (issuer-held monotonic narrowing overlay, effective-set funnels on every derivation, `authority_contained`, protected-event ingestion; the Protected Event Receipt rides the generic `ingestion` evidence kind rather than the draft's dedicated receipt object, tracked in #649) | `services/authorization-server/test/{containment,containment-pdp-e2e,protected-events}.test.ts`, `services/pdp/test/evaluate.test.ts`, `packages/mission-signals/test/containment-commit.test.ts` |
...
| `draft-mcguinness-mission-capability-binding` | `1c942878` (2026-08-21) | named in the architecture draft's Runtime-Enforced document map; only a nonconformant PEP `capability_source` placeholder exists (`services/mcp-payments/src/pep.ts`, wrong digest bytes and canonicalization) — full realization tracked in #657 |
...
- SUPERSESSION (2026-08-12, issue #475 / D67, PRs #477/#479/#478): the core's
  ...
  (PR #478). The deferred pin bump landed with the 2026-08-22 family audit: all three OAuth-binding rows pin `aa7ad294`, the carriage and anchor sections re-verified through that commit.
...
- FAMILY AUDIT (2026-08-22): full pin-drift pass over every in-repo row plus a
  coverage pass over the family. Bumped to draft HEAD after section-level
  review of each pin-to-HEAD diff: the three OAuth-binding rows (`aa7ad294`),
  authzen context.actor (`3730cc22`), attenuation (`3730cc22`), audit
  (`c7fa32ad`), management (`1c942878`), shaping (`3730cc22`), orchestration
  (`58c7a857`), work-products (`3730cc22`, a real commit pin replacing
  `-latest`), substrate (`af1782ed`, the new `#basic-gate` effective-active
  rule verified against `applyExpiry`), runtime-evidence (`41186df2`,
  `mission_reference_conflict` already realized). Containment gains its first
  row (`58c7a857`, partial); capability-binding enters Adopted-for-planning
  (#657). Deliberately NOT bumped, gap-first per the pin convention: harness
  `dc7a897` (Egress Evidence mission-reference shape, #649), cross-domain
  `dc7a897` (RAS local-token `client_id`, #650), expansion `dc7a897`
  (`mission_expires_at` #647; family-fallback lineage claim #651), the runtime
  decision/PEP-RS/read-binding rows `b5e35e2` (parameter-binding fail-open on
  absent amount + missing constraint fixtures, #656) and transaction-assurance
  `02d53dd` (idempotency claim states + active reconciliation, #648; S-6's
  disposition is stale, the draft moved the claim PDP-side). Cross-row impl
  gaps filed: `authority_source` (#646), `mission_expires_at` (#647), Decision
  Evidence `evidence_envelope` + Egress Evidence shape + Protected Event
  Receipt (#649). Spec-side inconsistencies filed: child-delegation's
  "interactive path is retained" vs the D62 removal (#653), orchestration's
  `compensates_decision_id` vs runtime's `compensates_evaluation_id` rename
  (#654). 29 stale `@spec` tags retagged onto the real
  expansion/child-delegation anchors (`creation-request-id` ->
  `creation-idempotency`, `creation-lookup-order` -> `creation-revalidation`,
  `nothing-to-expand` -> `verification-order`); the duplicated
  "Unified cross-enforcement evidence base" Notes paragraph removed.
