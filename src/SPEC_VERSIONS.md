...
| `draft-mcguinness-oauth-mission-cross-org-delegation` | `942d2b0` (2026-08-15) | `packages/mission-core/src/cross-org-presentation.ts` + `services/authorization-server/src/kernel/cross-org-chain.ts` + `services/authorization-server/src/kernel/attenuation.ts` (root mapping) + `services/authorization-server/src/adapters/cross-org-grant.ts` | `cross-org-delegation#chain-presentation`, `cross-org-delegation#actor-identity`, `cross-org-delegation#actor-evidence`, `cross-org-delegation#verification`, `cross-org-delegation#projection` | `packages/mission-core/test/cross-org-presentation.test.ts`, `services/authorization-server/test/cross-org-delegation.test.ts` |
| `draft-mcguinness-oauth-mission-transaction-authorization` | `700499b` (2026-08-17) | `packages/mission-core/src/txn-authorization.ts` + `services/authorization-server/src/adapters/transaction-authorization.ts` + `services/authorization-server/src/kernel/txn-workflow-store.ts` + `services/mcp-payments/src/{txn-challenge,txn-store,pep,server}.ts` | `txn-authorization#applicability`, `txn-authorization#resource-challenge`, `txn-authorization#challenge-redemption`, `txn-authorization#two-phase-expiry`, `txn-authorization#transaction-token`, `txn-authorization#offline-verification`, `txn-authorization#failure-semantics` | `packages/mission-core/test/txn-authorization.test.ts`, `services/authorization-server/test/{txn-endpoint,txn-authorization-e2e,child-delegation}.test.ts`, `services/mcp-payments/test/transaction.test.ts` |
