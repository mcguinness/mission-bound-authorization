...
| `draft-mcguinness-mission-authzen` (PDP request/decision) | `278dd777` (2026-09-05; phase carriers and the evaluation-context extension specified, implementation pending #252 C and #773) | `services/pdp` | `authzen#pdp-request` (envelope, context.audience rule), `authzen#denial-response`, `authzen#runtime-denial-classification` (incl. `authzen#failure-condition-coverage`: out_of_authority vs unsupported_authorization_type mapping), `authzen#materialization`, `authzen#response-context` (`evaluation_id`/`reason` additive alongside `decision_id`/`denial_reason`; permit `conditions` object with `parameter_digest`/`valid_until`/`use_limit` and conditional `action_phase` (phase implementation pending #252 C), replacing the flat `permit_expires_at`/`single_use` members; `use_limit: 1` set for both high-consequence classes present in this deployment) | `services/pdp/test/{evaluate,evaluate-decision,evaluate-fail-closed}.test.ts` |
| `draft-mcguinness-mission-runtime` (decision contract) | `73000de9` (2026-09-07; phase and evaluation-context implementation pending #252 C and #773) | `services/pdp` | abstract decision inputs, staleness bound, permit properties, discharged-entry exclusion (the `discharged` delta input and the authority_discharged denial) | `services/pdp/test/{evaluate,evaluate-decision-inputs,evaluate-classification,evaluate-decision,evaluate-compromise-resistant,evaluate-discharge}.test.ts` |
...
| `draft-mcguinness-mission-runtime-evidence` | `d42a5f2a` (2026-09-07; phase carriers and the evaluation-context extension specified, implementation pending #252 C and #773) | `services/pdp/src/{decision-evidence.ts,evaluate.ts,runtime-evidence-integrity.ts}` (Decision Evidence emission), `services/mcp-payments/src/evidence.ts` (retention, Refusal Records, Execution Evidence) | `runtime-evidence#decision-evidence-object`, `runtime-evidence#pre-decision-refusal`, `runtime-evidence#decision-evidence-integrity` (the emitter constructs the record on its own emission path; a receiving component verifies and retains it as received) | `services/pdp/test/{decision-evidence,runtime-evidence-integrity}.test.ts`, `services/mcp-payments/test/{decision-evidence-retention,refusal-evidence,enforcement}.test.ts` |
