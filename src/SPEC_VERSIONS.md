...
|---|---|---|---|---|
| `draft-mcguinness-mission-control-plane` | `6dbc9eb9` (generated status only; normative text unchanged from `efb2a01b`) | `packages/store`, `packages/mission-signals`, `services/authorization-server` | `control-plane#serialization`, `control-plane#isolation`, `control-plane#fresh-observation`; partial single-process transaction, issuer-namespace and Status List republication coverage only; the observation/commit watermark, asynchronous issuance and generalized durable fan-out remain incomplete; deployment limits in `docs/control-plane-deployment.md` | `packages/store/test/serialization.test.ts`, `packages/mission-signals/test/durable-delivery.test.ts`, `services/authorization-server/test/{control-plane-faults,child-delegation,expansion-endpoint,status-list}.test.ts` |
