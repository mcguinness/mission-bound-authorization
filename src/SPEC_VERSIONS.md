...
|---|---|---|---|---|
| `draft-mcguinness-mission-control-plane` | `ae362420` (generated status only; normative text unchanged from `efb2a01b`) | `packages/store`, `packages/mission-signals`, `services/authorization-server` | `control-plane#serialization`, `control-plane#isolation`, `control-plane#fresh-observation`, `control-plane#fanout`, `control-plane#tombstones`, `control-plane#deployment-declaration`; single-process transaction, issuer-namespace and Status List republication coverage, durable lifecycle fan-out with per-subscriber acceptance and boot replay, composed terminal tombstones with permanent identifier nonreuse, the transactional Status observation carrying its own `(version, commit)` watermark with an optional stricter refusal policy, the derivation counter's durable operation and artifact identity with never-refund recovery states, and the lifecycle response signing boundary (claim in the transition transaction, exact response finalized and recovered outside it); the Status conditional-request surface, transactional artifact issuance, refusal under uncertain recovery and multi-process claiming remain incomplete; deployment limits in `docs/control-plane-deployment.md` | `packages/store/test/serialization.test.ts`, `packages/mission-signals/test/durable-delivery.test.ts`, `services/authorization-server/test/{control-plane-faults,control-plane-fanout,control-plane-responses,child-delegation,expansion-endpoint,status-list}.test.ts` |
| `draft-mcguinness-mission-runtime` and `draft-mcguinness-oauth-mission-status` (operational declarations) | `cbbdcd7e` (runtime); `d29d3e27` (Status; generated coverage metadata only) | `config/enforcement-scope.json`, `services/pdp/src/{runtime-posture,policy}.ts`, `services/mcp-payments/src/server.ts`, `services/authorization-server/src/adapters/provider.ts`, `packages/demo-data/src/index.ts` | `runtime#ride-through`, `runtime#runtime-operational`, `status#status-operational`, `status#as-metadata`; per-class bounds consumed from the published statement, issuer ceiling, recovery objective and deny-only posture; remote wiring pending #310 B | `services/pdp/test/runtime-posture.test.ts`, `services/pdp/test/evaluate-classification.test.ts`, `services/mcp-payments/test/runtime-posture-metadata.test.ts`, `services/authorization-server/test/tracer.test.ts` |
| `draft-mcguinness-mission-runtime-evidence` (receipt verification) | `3c87de67` (generated status only; normative text unchanged from `d42a5f2a`) | `services/mcp-payments/src/mission-receipt.ts`, PDP Enforcement Scope Statement receipt-issuer extension | `runtime-evidence#mission-receipt-verification`; scope-designated issuer, total shape refusal, ordered envelope/source checks and selected projection equality; chaining, richer conditional content and verified producer assembly remain incomplete | `services/mcp-payments/test/mission-receipt.test.ts` |
| `draft-mcguinness-mission-runtime-evidence` (integrity verification) | `3c87de67` (generated status only; normative text unchanged from `d42a5f2a`) | `services/pdp/src/runtime-evidence-integrity.ts`, PEP configured key resolver and console read model | `runtime-evidence#decision-evidence-integrity`, `runtime-evidence#evidence-integrity-signing-keys`, `runtime-evidence#security-considerations`; total ordered refusal, compromise gating, exact configured scope and conservative Decision-only outcome classification; published JWKS and audit TLS/encrypted persistence remain incomplete | `services/pdp/test/runtime-evidence-integrity.test.ts`, `services/mcp-payments/test/evidence-key-resolution.test.ts`, `services/console-bff/test/activity-log.test.ts` |
| `draft-mcguinness-mission-runtime-evidence` (audit projection) | `3c87de67` (generated status only; normative text unchanged from `d42a5f2a`) | `services/pdp/src/{decision-evidence,evaluate}.ts`, `services/mcp-payments/src/{server,pep}.ts` | `runtime-evidence#decision-evidence-object`; runtime allowlists, verified credential issuer/expiry, normalized conditions and permit-entry guards; complete-object and whole-request-digest assertions remain partial | `services/pdp/test/decision-evidence.test.ts`, `services/mcp-payments/test/decision-evidence-retention.test.ts` |
| `draft-mcguinness-mission-runtime-evidence` (contributing constraints) | `3c87de67` (generated status only; normative text unchanged from `d42a5f2a`) | `services/pdp/src/{evaluate,mas-join,decision-evidence}.ts`, receipt verifier | `runtime-evidence#decision-evidence-object`; PDP-owned trace of reached entries and keys; constraint failures use `parameter_violation` on the live response and evidence, with keys separate; unfamiliar reasons remain denial | `services/pdp/test/decision-evidence.test.ts`, `services/mcp-payments/test/mission-receipt.test.ts`, amount and downstream denial-code regressions |
| `draft-mcguinness-mission-runtime` (approval-resolution boundary) | `73000de9` (2026-09-07) | `services/authorization-server/src/adapters/{provider,approval-resolution}.ts`, trusted demo console | `runtime#action-approval`, `runtime#agent-isolated-evidence-emission`, `runtime#negative-conformance`; surface-established identity with the matching core sentence already landed in #761 | `demo/test/approval-resolution-identity.test.ts`, `demo/test/approval-client-separation.test.ts`, `services/mcp-payments/test/approval-resolution-boundary.test.ts` |
| `draft-mcguinness-mission-capability-binding` | `1c84c745` (2026-09-04; generated status only, normative text unchanged from `8d470e4`) | `packages/mission-core`, `services/authorization-server` (trusted resolver, approval/expansion/template), `services/mcp-payments` (catalog, PEP), `services/pdp` (verification and evidence) | `capability-binding#capability-source-binding`, `capability-binding#context-capability-source`, `capability-binding#capability-extraction`, `capability-binding#capability-verification`, `capability-binding#capability-drift-reason`, `capability-binding#conformance`; selected-definition trust unit; whole-catalog comparison fixture-only; committed per-action shape is the implementation-defined member from #752 | `packages/mission-core/test/capability-binding.test.ts`, `services/authorization-server/test/{capability-binding,capability-resolver}.test.ts`, `services/mcp-payments/test/capability-binding.test.ts`, `services/pdp/test/{evaluate-capability-binding,decision-evidence}.test.ts` |
| `draft-mcguinness-oauth-mission` | `0dfd086` (2026-09-07; informative derivation-policy appendix only, normative text unchanged from `3b0de26`) | `packages/mission-core` | `mission#integrity-anchors`, `mission#canonicalization`, `mission#test-vectors`, `mission#authority-sources` | `packages/mission-core/test/anchors.test.ts` |
| `draft-mcguinness-oauth-mission` | `0dfd086` (2026-09-07; informative derivation-policy appendix only, normative text unchanged from `3b0de26`) | `services/authorization-server` (kernel + adapters) | `mission#submission-via-par`, `mission#mission-intent`, `mission#authorization-derivation`, `mission#subset`, `mission#integrity-anchors`, `mission#the-mission-claim`, `mission#lifecycle`, `mission#introspection`, `mission#as-metadata`, `mission#mission-record`, `mission#grant-binding`, `mission#authority-sources`, `mission#approval-event` (the AS-established effective `expires_at`: the `resolveEffectiveExpiry` hook, the `insertRecord` creation-transaction invariant, the already-past intake refusal, and `mission_expires_at` on every Mission-creating success body) | `services/authorization-server/test/{kernel,tracer,authorization-endpoint,authority-source,rar-carriage}.test.ts` |
| `draft-mcguinness-oauth-mission-status` | `9311ba74` (2026-09-08; the signed Status response now carries `carried_to`, normative text unchanged from `49fc10f`) | `services/authorization-server` (kernel + adapters) | `status#legal-transitions`, `status#state-machine`, `status#mission-status-response` (incl. the CONDITIONAL `carried_to` correlation on an old child's `cascaded` state, present only when a replacement committed), `status#mission-status-caching` (the advertised-ceiling cap inside `signedStatus`), `status#mission-status-anti-oracle`, `status#mission-status-errors`, `status#mission-status-authentication`, `status#idempotency`, `status#as-metadata`, `status#status-operational` | `services/authorization-server/test/{kernel,tracer,carryover}.test.ts` |
...
| `draft-mcguinness-oauth-mission-signals` | `9311ba74` (2026-09-08; the lifecycle SET now carries `carried_to`, normative text unchanged from `edb5f6b`) | `packages/mission-signals` + `services/authorization-server` (kernel commit hook subscriber) | `signals#lifecycle-event` (incl. the CONDITIONAL `carried_to` correlation on an old child's `cascaded` event, relayed from the kernel commit and omitted when no replacement committed), `signals#set-protection`, `signals#consumer-behavior`, `signals#discharge-compatibility` | `packages/mission-signals/test/{signals,authority-changed,discharge-signals,carryover-correlation}.test.ts` |
...
| `draft-mcguinness-mission-authzen` (PDP request/decision) | `278dd777` (2026-09-05; the phase carriers are implemented, #252 C; the evaluation-context extension stays specified with implementation pending #773) | `services/pdp` | `authzen#pdp-request` (envelope, context.audience rule), `authzen#denial-response`, `authzen#runtime-denial-classification` (incl. `authzen#failure-condition-coverage`: out_of_authority vs unsupported_authorization_type mapping), `authzen#materialization`, `authzen#response-context` (`evaluation_id`/`reason` additive alongside `decision_id`/`denial_reason`; permit `conditions` object with `parameter_digest`/`valid_until`/`use_limit` and conditional `action_phase`, replacing the flat `permit_expires_at`/`single_use` members; `use_limit: 1` set for both high-consequence classes present in this deployment), `authzen#context-action-phase` (the PEP-derived request mirror: validated against the closed set, a value outside it refused as `out_of_authority`, and the validated phase echoed as the permit condition and recorded on Decision Evidence for a permit and a denial alike, malformed input omitted rather than cast) | `services/pdp/test/{evaluate,evaluate-decision,evaluate-fail-closed}.test.ts` |
| `draft-mcguinness-mission-runtime` (decision contract) | `73000de9` (2026-09-07; the phase carriers and the state-view permit cap are implemented, #252 C; evaluation-context implementation pending #773) | `services/pdp` | abstract decision inputs, staleness bound, permit properties, discharged-entry exclusion (the `discharged` delta input and the authority_discharged denial), `runtime#state-freshness` (one permit-deadline calculation, `permit-deadline.ts`: a permit expires no later than the accepted state observation plus the published class bound, with the class bound alone where no observation is available, no cap under a declared `none` posture, a future-dated observation clamped to the decision instant, and an already-reached ceiling denying `stale_state` rather than issuing a pre-expired permit; the authority, credential and policy ceilings extend that same helper under #594 W4-13) | `services/pdp/test/{evaluate,evaluate-decision-inputs,evaluate-classification,evaluate-decision,evaluate-compromise-resistant,evaluate-discharge,permit-deadline}.test.ts` |
...
| `draft-mcguinness-mission-authzen` (PEP envelope + evidence) | `3f033485` (2026-09-08; the failure-condition table gains the post-permit duplicate counterparts, the two-carrier capability rule and five pre-decision rows. The phase carriers are implemented, #252 C; the evaluation-context extension stays specified with implementation pending #773) | `services/mcp-payments/src/pep.ts` | envelope build (context.actor, parameter_digest), Refusal Records; `authzen#response-context` `decision_evidence` (the PDP-emitted signed record, verified and retained by the PEP, never authorization to act) and `use_limit` (the PDP declares it; this PEP owns the consumed-identifier store); response-context conditions parsing (`decision.context.conditions` recognized-member check scoped to `conditions` only, never the whole top-level context; `decision.context.obligations` presence is an effective deny, `unfulfillable_obligation`, disposed of as suppressed Execution Evidence with `obligation_outcomes`); `authzen#failure-condition-coverage` (exactly one carrier per condition, chosen by call site: the same capability-source reason on a Refusal Record pre-decision and on Execution Evidence at use, and a post-permit duplicate on Execution Evidence while the wire keeps the mandated `duplicate_suppressed` diagnostic) | `services/mcp-payments/test/{enforcement,pep-fail-closed,decision-evidence-retention,capability-binding,transaction,mcp-channel}.test.ts` |
...
| `draft-mcguinness-mission-runtime` (transaction-assurance tier) | `02d53dd` | `services/mcp-payments` (transaction, connectors, reconcile) | single-use permits, execution leases, operation state machine (D36), Execution Evidence, outcome reconciliation; `runtime#execution-reverification` (the execution lease maximum is PUBLISHED in the Enforcement Scope Statement's `extensions.transaction_assurance` declaration, with its units and its consuming PEP named and both validated at load, and the local lease is that bound capped by the permit's own `valid_until`, stamped as an absolute end so it can never extend authorization); `runtime#idempotency` (the operation key carries the crossing's phase where the profile declares one, so crossings sharing an action identifier and a parameter digest claim distinct keys through the state machine, the connector's idempotency key, the single-use consumption lookup and the evidence join key) | `services/mcp-payments/test/transaction.test.ts`, `services/mcp-payments/test/compound-actions.test.ts` |
...
| `draft-mcguinness-mission-runtime` (read binding + decision-output polarity) | `73000de9` (2026-09-07; the phase binding is implemented, #252 C; evaluation-context implementation pending #773) | `services/mcp-payments/src/pep.ts`, `services/mcp-payments/src/server.ts`, `services/mcp-payments/src/effective-params.ts`, `services/pdp/src/evaluate.ts` | `runtime#read-binding` (list_invoices binds a requested vendor_id through the ordinary vendor-constraint check, and binds its absence to a normalized {vendor_scope, vendor_scope_source} Operation Profile whose parameter_digest is reverified immediately before execution; a multi-vendor collection is checked member-by-member against Resource policy via `resource.properties.vendor_ids`, evaluateInner step 6a, not just one representative object), `runtime#decision-output` (a permit's `decision.context.conditions` member outside {parameter_digest, valid_until, use_limit, action_phase, evaluation_context_digest} refuses with zero effect; this implementation's recognized set is {parameter_digest, valid_until, use_limit, action_phase}, so it still refuses a presented `evaluation_context_digest` until #773's extension is implemented, emitting none; an obligation's mere presence is an effective deny), `runtime#compound-actions` (three served crossings of `payments:payment.execute` on one action identifier and one normalized effect form; the phase comes from the trusted Operation Profile and never from an argument, and `Pep.verifyPermitAtUse` compares the bound phase and `valid_until` in front of EVERY crossing on all three dispatch paths, before permit redemption, then re-checks the time-sensitive conditions immediately before release, emitting `phase_mismatch` or `permit_expired` through the one post-permit suppression writer) | `services/mcp-payments/test/pep-fail-closed.test.ts`, `services/pdp/test/evaluate-decision.test.ts` |
...
| `draft-mcguinness-oauth-mission-expansion` | `9311ba74` (2026-09-08; successor-identifier reservation and carryover completion implemented, normative text unchanged from `8d470e4`) | `services/authorization-server/src/kernel/expansion.ts`, `services/authorization-server/src/kernel/carryover.ts` | successor Mission, `predecessor` member, the bare same-issuer `related_to` correlation, supersede-on-redemption, approved_until bounding, `expansion#successor-expiry` (bounded by the predecessor's `expires_at` unless a recorded, disclosed, policy-permitted extension ceiling applies, and a recorded extension never reaches a carried replacement; `mission_expires_at` on the completing body; completion inherits the atomic creation-time check and denies rather than creating a successor), `expansion#deferred-window` and `expansion#superseded-state` (a Child Mission Carryover batch commits inside the completion transaction, so the post-supersession cascade has nothing left) | `services/authorization-server/test/{arop,carryover,expansion-endpoint}.test.ts` |
| `draft-mcguinness-oauth-mission-child-delegation` | `9311ba74` (2026-09-08; Child Mission Carryover implemented, normative text unchanged from `01306cc`) + AS wire (branch `feat/child-delegation-wire`, 2026-08-05) | `services/authorization-server/src/kernel/child-delegation.ts` (+ kernel `cascadeChildren`/`findChildren`/`descendantsOf`/`carryTerminalInCallerTx`/suspend-projection, `parent`/`ParentRef`, `related_to`/`carried_to`), `services/authorization-server/src/kernel/carryover.ts`; AS wire in `services/authorization-server/src/adapters/provider.ts` (`extraParams` `parent`/`parent_token`/`child_actor`/`carryover_replacement`, creation + child-redemption grants on `/token`, discovery flag) and `services/authorization-server/src/adapters/child-grant.ts` (child-bound grant) | `child-delegation#child-creation`, `#child-client-identity`, `#request-processing`, `#parent-member`, `#strict-subset`, `#cascade`, `#discovery`, `#grant-type` (kernel flow, terminal cascade, suspend-projection, fan-out accounting; PAR wire params, parent resolve-only, child-bound RFC 7523 grant issuance, discovery metadata all realized; child redeeming AS ITSELF at `/token` (PR4b) and creation relocated onto `/token` as an impl-local grant retiring `POST /child-missions` (PR4c); the response shape's REQUIRED `mission_expires_at` carried on both the creation and recovery bodies; fully wired, single-issuer); `#carryover`, `#carryover-manifest`, `#carryover-cas`, `#carryover-generations`, `#carryover-no-reset`, `#carryover-records`, `#carryover-commit`, `#carryover-evidence` (the capability-gated batch: read-only preparation with reserved identifiers, the JCS commitment construction for `manifest_hash` and the deterministic per-child approval event, a transaction-only application with descendant-set membership plus per-row compare-and-set, fresh `direct`-basis replacement records with re-run source activation and ceiling, the whole old subtree terminated through one carried-terminal funnel, and the retained map with synchronously signed Carryover Evidence retrievable by the replacement's own child actor; `#carryover-generations`' actor/children-control refusal arms and the external meter/latch transfer remain partial, and no policy-adjudicated completion path exists for `#carryover`'s drawdown prohibition to refuse from) | `services/authorization-server/test/{child-delegation,child-delegation-endpoint,carryover,expansion-endpoint}.test.ts`, `packages/mission-signals/test/carryover-correlation.test.ts` |
...
| `draft-mcguinness-oauth-mission-cross-domain` | `ca6bebfc` | `services/authorization-server/src/kernel/cross-domain.ts`, `services/ras` | ID-JAG grant issuance (audience-scoped, PoP, one-time, mission-preserving), RAS validation and required versioned local-policy ceiling (per-action narrowing after entitlement, before JTI consumption) on base and continuation grants; derivation evidence/local JTI remains a separately tracked SHOULD; RAS client authentication at redemption via its own jkt-keyed registration (`registeredClients`/`registerClient`, invalid_client on an unrecognized key), local-token `client_id` identifying the authenticated redeeming client (S-12), never the grant's own `client_id` (the originating agent) | `services/ras/test/mapping-entitlement.test.ts`, `services/mcp-saas/test/cross-domain.test.ts`, `services/authorization-server/test/continuation-grant.test.ts` |
...
| `draft-mcguinness-mission-orchestration` | `24df23f6` (2026-08-26) | `packages/orchestration` + `packages/mission-core` (anchors `UNWIND_PLAN_TYP`) | `orchestration#reversibility`, `orchestration#unwind-plan`, `orchestration#unwind-plan-integrity`, `orchestration#state-change-behavior`, `orchestration#compensation`, `orchestration#compensation-authority`, `orchestration#orchestration-evidence` | `packages/orchestration/test/orchestration.test.ts` |
...
| `draft-mcguinness-oauth-mission-transaction-authorization` | `12b540f5` (2026-08-20) | `packages/mission-core/src/txn-authorization.ts` | `txn-authorization#resource-challenge`, `#challenge-redemption`, `#transaction-token`, `#offline-verification`, `#two-phase-expiry`, `#failure-semantics` | `packages/mission-core/test/txn-authorization.test.ts` |
| `draft-mcguinness-oauth-mission-transaction-authorization` | `12b540f5` (2026-08-20; #subject-establishment and #subject-namespaces are implemented as the SubjectNamespacePolicy seam: configured issuer acceptance, same-issuer copy by default, injective mapping through the seam, source identity + policy pinned on the workflow, completion re-resolution on both passes -- no deployment foreign-mapping registry ships, the default policy accepts only this AS's own issuer) | `services/authorization-server/src/adapters/transaction-authorization.ts` + `services/authorization-server/src/kernel/txn-workflow-store.ts` + `services/authorization-server/src/kernel/transaction-token.ts` (the transaction-token mint) + `services/authorization-server/src/kernel/txn-challenge.ts` (the challenge verifier) + `services/authorization-server/src/kernel/operation-profile.ts` + `services/authorization-server/src/kernel/derive.ts` + `services/authorization-server/src/kernel/child-delegation.ts` + `services/authorization-server/src/kernel/types.ts` + `services/authorization-server/src/adapters/provider.ts` + `services/authorization-server/src/index.ts` | `txn-authorization#applicability`, `#resource-challenge`, `#challenge-redemption`, `#subject-establishment`, `#subject-namespaces`, `#two-phase-expiry`, `#transaction-token`, `#offline-verification`, `#failure-semantics` | `services/authorization-server/test/{txn-endpoint,txn-authorization-e2e,child-delegation}.test.ts` |
| `draft-mcguinness-oauth-mission-transaction-authorization` | `12b540f5` (2026-08-20) | `services/mcp-payments/src/{txn-challenge,txn-store,pep,server,mcp-transport,mcp-http-transport,resource-metadata}.ts` | `txn-authorization#resource-challenge`, `#transaction-token`, `#two-phase-expiry`, `#offline-verification`, `#challenge-redemption` | `services/mcp-payments/test/transaction.test.ts` |
| `draft-mcguinness-oauth-mission-transaction-authorization` | `700499b` (2026-08-17; #applicability only, unaffected by #588's #challenge-redemption/#transaction-token edits) | `services/pdp/src/evaluate.ts` + `services/pdp/src/policy-view.ts` | `txn-authorization#applicability` | `services/pdp/test/evaluate.test.ts` |
| `draft-mcguinness-oauth-mission-transaction-authorization` | `12b540f5` (2026-08-20) | `services/access-request/src/index.ts` | `txn-authorization#challenge-redemption`, `#offline-verification` | `services/access-request/test/txn.test.ts` |
...
| `draft-mcguinness-mission-runtime-evidence` | `18826b1d` (2026-09-09; generated status only, normative text unchanged from `ce18f104`: the post-permit disposition path of five added Execution Evidence `error` values, five added pre-decision `denial_reason` values, the duplicate-identifier separation and the enumerated-value-not-diagnostic rule. Phase carriers and the evaluation-context extension specified, implementation pending #252 C and #773; the unavailable-target `effective_parameter_digest` encoding is open with #594 and one exit ships recordless because of it) | `services/pdp/src/{decision-evidence.ts,evaluate.ts,runtime-evidence-integrity.ts}` (Decision Evidence emission), `services/mcp-payments/src/evidence.ts` (retention, Refusal Records, Execution Evidence), `services/mcp-payments/src/evidence-retention.ts` (the durable record, sequence and published-key store: the audit window, key retirement anchored on the last artifact signed, and the resolvers verification takes), `services/mcp-payments/src/{pep,server,transaction}.ts` (one post-permit suppression writer, per-attempt execution identity, call-site classification) | `runtime-evidence#decision-evidence-object`, `runtime-evidence#pre-decision-refusal`, `runtime-evidence#execution-evidence-object`, `runtime-evidence#decision-evidence-integrity` (the emitter constructs the record on its own emission path; a receiving component verifies and retains it as received), `runtime-evidence#receipt-retention`, `runtime-evidence#evidence-integrity-signing-keys` (a retired key stays resolvable in the published set for the window past the last artifact it signed), `runtime-evidence#receipt-verification`, `runtime-evidence#receipt-chaining` (retained evidence and retained predecessors are what the verifier resolves; the Evidence capability itself stays unclaimed by the shipped deployment) | `services/pdp/test/{decision-evidence,runtime-evidence-integrity}.test.ts`, `services/mcp-payments/test/{decision-evidence-retention,refusal-evidence,enforcement,pep-fail-closed,transaction,capability-binding,mission-receipt}.test.ts` |

## Adopted for planning, not yet implemented (pins from the pre-flight spike and later dispositions)

| Spec | Pinned version | Lands in |
|---|---|---|
| `draft-mcguinness-mission-runtime` (Evaluation-Context Binding) | `73000de9` | #773 optional per-class extension: descriptor-bound authoritative context, persistent salt state, existing approval/operation binding preservation and verified/enforced distinction; implementation pending |
| `draft-mcguinness-mission-authzen` (evaluation-context carriers) | `278dd777` | #773 request descriptor/context digest and live permit condition; implementation pending |
| `draft-mcguinness-mission-runtime-evidence` (evaluation-context projection) | `d42a5f2a` | #773 context digest/descriptor, execution digest pair and target_drift; implementation pending, no claim enabled |
