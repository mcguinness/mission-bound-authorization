...
|---|---|---|---|---|
| `draft-mcguinness-mission-control-plane` | `3922b47b` (generated status only; normative text unchanged from `efb2a01b`) | `packages/store`, `packages/mission-signals`, `services/authorization-server` | `control-plane#serialization`, `control-plane#isolation`; partial single-process transaction and issuer-namespace coverage only; asynchronous issuance and generalized durable fan-out remain incomplete; deployment limits in `docs/control-plane-deployment.md` | `packages/store/test/serialization.test.ts`, `packages/mission-signals/test/durable-delivery.test.ts`, `services/authorization-server/test/{control-plane-faults,child-delegation,expansion-endpoint}.test.ts` |
