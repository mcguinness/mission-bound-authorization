...
| `draft-mcguinness-mission-runtime` (read binding + decision-output polarity) | `b5e35e2` (2026-08-18) | `services/mcp-payments/src/pep.ts`, `services/mcp-payments/src/server.ts` | `runtime#read-binding` (list_invoices binds a requested vendor_id through the ordinary vendor-constraint check, and binds its absence to the entry's own vendor scope, never the unconstrained store), `runtime#decision-output` (a permit's decision.context member outside the recognized conditions + metadata enumeration refuses with zero effect, never ignored) | `services/mcp-payments/test/pep-fail-closed.test.ts` |
