...
| `draft-mcguinness-oauth-mission-status` | `49fc10f` (2026-08-26) | `services/authorization-server` (kernel + adapters) | `status#legal-transitions`, `status#state-machine`, `status#mission-status-response`, `status#mission-status-caching` (the advertised-ceiling cap inside `signedStatus`), `status#mission-status-anti-oracle`, `status#mission-status-errors`, `status#mission-status-authentication`, `status#idempotency`, `status#as-metadata`, `status#status-operational` | `services/authorization-server/test/{kernel,tracer}.test.ts` |
...
| `draft-mcguinness-oauth-mission-signals` | `edb5f6b` (2026-08-13) | `packages/mission-signals` + `services/authorization-server` (kernel commit hook subscriber) | `signals#lifecycle-event`, `signals#set-protection`, `signals#consumer-behavior`, `signals#discharge-compatibility` | `packages/mission-signals/test/{signals,authority-changed,discharge-signals}.test.ts` |
