...
| `draft-mcguinness-oauth-mission-cross-domain` | `d8c5b459` | `services/authorization-server/src/kernel/cross-domain.ts`, `services/ras` | ID-JAG grant issuance (audience-scoped, PoP, one-time, mission-preserving), RAS validation; RAS client authentication at redemption via its own jkt-keyed registration (`registeredClients`/`registerClient`, invalid_client on an unrecognized key), local-token `client_id` identifying the authenticated redeeming client (S-12), never the grant's own `client_id` (the originating agent) | `services/mcp-saas/test/cross-domain.test.ts`, `services/authorization-server/test/continuation-grant.test.ts` |
...
| `draft-mcguinness-mission-capability-binding` | `8d470e4` (2026-08-25) | Target for `services/mcp-payments/src/pep.ts` (validating-server recording, PEP `context.capability_source` presentation) and `services/pdp/src/evaluate.ts` (`capability_drift` verification), once #657 PR A defines the committed per-action shape the draft's presented `context.capability_source` object needs (the draft specifies the presented object and its per-capability JCS extraction rule, but names no committed member on the Authority Set entry). A prior `pep.ts` placeholder presenting a whole-server `JSON.stringify` digest as `capability_source` was removed, not gated, since no committed shape exists yet to gate it against (#657). |
