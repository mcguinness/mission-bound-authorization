...
| `draft-mcguinness-oauth-mission-cross-domain` | `15810598` | `services/authorization-server/src/kernel/cross-domain.ts`, `services/ras` | ID-JAG grant issuance (audience-scoped, PoP, one-time, mission-preserving), RAS validation, local-token `client_id` identifying the redeeming destination client (S-12, `RasConfig.localClientId`), never the grant's own `client_id` (the originating agent) | `services/mcp-saas/test/cross-domain.test.ts`, `services/authorization-server/test/continuation-grant.test.ts` |
