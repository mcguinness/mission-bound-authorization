...
describe("Mission Receipt build + verify", () => {
  ...
  it("an unknown denial reason remains a deny and can never become an execution authorization", async () => {
    ...
    const receipt = await buildAndSignMissionReceipt({ kind: "decision", mission: MISSION, decisionEvidence: decision }, "receipts.example.com", RECEIPT_SIGNER);
    expect(await verifyMissionReceipt(receipt, resolverFor({ decision }), receiptIssuers, resolveEvidenceKey)).toEqual({ valid: true });
    ...
    expect(await verifyMissionReceipt(invalid, resolverFor({ decision, execution }), receiptIssuers, resolveEvidenceKey)).toMatchObject({ valid: false });
  });
