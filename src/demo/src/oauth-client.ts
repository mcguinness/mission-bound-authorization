 ...
 * authorization server: PAR -> authorize -> independently resolved approval ->
...
export interface IssueArtifacts {
  ...
  decide: { request: { decision: string }; code: string };
...
export function jarClosures(cookies: CookieJar): { cookieHeader: () => string; storeCookies: (res: Response) => void } {
...
/**
 * Redeem a code after the independent approval surface has resolved it.
 * This client helper possesses no approver credential or resolution callback.
 * DPoP is introduced only at /token, so the keypair is generated here. Returns
 * only the client-owned IssuedMission credential material.
 */
export async function redeemMissionApproval(
  asUrl: string,
  agentClientJwk: Record<string, unknown>,
  opts: { code: string; par?: SubmittedApproval["par"] },
  ...
  return {
    ...
    artifacts: {
      ...
      decide: { request: { decision: "approve" }, code },
