...
: REQUIRED. A string. An RFC 3339 {{RFC3339}}
  ...
  already-past value is refused with `invalid_request` at submission
  acceptance (the member rides `mission_intent`, and the request may
  carry no `authorization_details` for an
  ...
  expiry atomically at the commit ({{approval-event}}).
...
Alongside it, this document defines `mission_expires_at`: an RFC
3339 string carrying the Mission Record's effective `expires_at`
...
creation replay deduplicated under `approval_event_id`
({{mission-record}}) returns the committed effective value
unchanged. On OAuth token responses the member is additionally
registered as a token-endpoint response parameter ({{iana}}). It is
informational in the same way: presenting it authorizes nothing.
