...
MUST provision that standing together with the Mandate: grant the
recipient access to the Mission Status operation for that `mission.id`
({{I-D.draft-mcguinness-oauth-mission-status}}), enroll it in a
Lifecycle Signals stream
({{I-D.draft-mcguinness-oauth-mission-signals}}), or offer a
state-only surface that returns the Mission's lifecycle state and
nothing else. Minting evidence a cross-domain recipient can verify but
never refresh leaves the required check unreachable.
...
when `mandate_exp` is absent. Routine rotation retires a key from
signing, never from resolvability within that bound: a key retired
from signing stays published as valid for verification.

Revocation for compromise is distinct from routine retirement. When a
signing key is known or suspected compromised, the issuer MUST publish
it as revoked, not merely retired: either remove it from the
verify-as-valid key material outright, or mark it there with a
compromise time. A key removed outright can no longer verify any
Mandate, genuine or forged; a key marked with a compromise time stays
resolvable, so genuine Mandates it signed before that time remain
verifiable, and a verifier MUST reject any Mandate under that key
whose `iat` is at or after the compromise time (the invalid class of
{{failures}}). A key-holding attacker can backdate `iat` to before the
compromise time, so the compromise time alone does not separate a
genuine pre-compromise Mandate from a backdated forgery; audit
registration and its Receipt do ({{audit-evidence}},
...
- it does not match and undisclosed array-element digests remain: the
  ...
  MUST NOT recompute the anchor as if the set were complete and MUST
...
: The artifact verifies, but an SD-JWT presentation discloses only
  ...
  normally. A verifier that needs the full Authority Set MUST obtain a
  fuller presentation, MUST NOT recompute the anchor as if the set
  were complete, and MUST NOT treat the mismatch as tampering.
...
step of {{verification}}. The key-retention rule of {{minting}} keeps
genuine Mandates verifiable for their evidence lifetime, so an issuer
cannot recover by simply dropping the key from the published set
without also breaking the genuine Mandates that key signed.

The issuer bounds this by publishing the compromised key as revoked,
distinct from routine retirement ({{minting}}): removed from the
verify-as-valid material, or marked with a compromise time so a
verifier rejects any Mandate under it minted at or after that time
(step 3 of {{verification}}). Because a key-holding attacker can
backdate `iat` to before the compromise time, that cutoff alone does
not catch a backdated forgery. Audit registration ({{audit-evidence}})
does: a genuine Mandate has an independent, timestamped existence proof
predating the compromise, and a forged one either goes unregistered or
carries a registration time after the compromise, a permanent,
attributable trace. A verifier relying on a Mandate for a
high-consequence decision MUST confirm that existence proof (step 9 of
{{verification}}), and a deployment whose Mandates feed such decisions
MUST register them.
